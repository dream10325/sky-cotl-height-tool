const translations = {
    ...
    'ja': {
        ...
        inst_4: "<b>方法1（推奨）：スクリーンショットを直接アップロード</b><br>このページに戻り、「画像を選択」ボタンをクリックするか、スクリーンショットファイルをアップロードエリアにドラッグ＆ドロップします。ツールが自動的に画像を認識し、計算を完了します。",
        inst_5: "<b>方法2：URLを貼り付け</b><br>別のスマートフォンやウェブQRコードスキャナーでスクリーンショットを読み取ります。<code>https://sky.thatg.co/o=</code>で始まるURLを取得し、<b>URL全体をコピー</b>して下の入力欄に貼り付けてください。",
        faq_title: "よくある質問（FAQ）",
        faq_q1: "Q: ウェブサイトで身長が計算できません。",
        faq_a1: "A: サイトを更新中の可能性があります。数分待っても解決しない場合は、不具合報告をお願いします。",
    ...
    dom.calculateBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            if (result.error) {
                ...
                if(dom.potionExtremeNotice) dom.potionExtremeNotice.style.display = 'none';
                ...
                animateValue(dom.resCurrent, 0, result.current, 1000);
                animateValue(dom.resTallest, 0, result.tallest, 1000);
                animateValue(dom.resShortest, 0, result.shortest, 1000);
                if (dom.resJson) {
                    dom.resJson.textContent = JSON.stringify(result.json, null, 2);
                    // mark container as having value for styling
                    const container = document.querySelector('.result-item.json-output');
