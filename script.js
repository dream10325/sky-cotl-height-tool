...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let lastResult = null; // 【圖片錯誤修復】儲存最新的計算結果物件
    let potionCounter = 0;
    ...
    const drawCanvasContent = (ctx, canvas) => {
        // 【圖片錯誤修復】直接從 'lastResult' 物件讀取精確數值，而不是從正在動畫的DOM讀取
        if (!lastResult) {
        ...
        ctx.globalAlpha = 0.8;
        ctx.textAlign = 'center'; // 確保 footer text置中
    ...
    dom.calculateBtn.addEventListener('click', () => {
        ...
        if (results.error) {
            ...
            lastResult = results; // 【圖片錯誤修復】儲存成功的計算結果
