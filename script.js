...
function decodeAndCalculate(rawData) {
    try {
        ...
        // 【錯誤修復】使用更寬鬆的 Regex 來抓取 height 和 scale 數值
        const heightRegex = /h?eigh[^:=\d\-.eE]+[:=]\s*(-?\d*\.\d+)/;
        const scaleRegex = /scale[^:=\d\-.eE]+[:=]\s*(\d*\.\d+)/;

        const heightMatch = decodedText.match(heightRegex);
        const scaleMatch = decodedText.match(scaleRegex);

        if (!heightMatch || !scaleMatch) {
            console.error("Failed to match height or scale", {heightMatch, scaleMatch});
